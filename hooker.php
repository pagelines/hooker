...
class PLHooks {
	...
	function render_admin_page() {
		...
		echo '<option disabled="disabled">PageLines Hooks</option>';
		...
		echo '<option disabled="disabled">PageLines Sections</option>';
		...
		if( ! empty( $options ) ) {
			...
			echo "<option name='none' id='none' value='none'>Edit hook...</option>";
	...
	function draw_options() {
		...
		foreach( $options as $o => $d ) {
			...
			if( isset( $d['enabled'] ) && 'on' === $d['enabled'] )
				...
				$check = 'unchecked';
			echo "<br /><input type='checkbox' name='enabled' " . $check . " /> Enable this hook.";			

			if ( defined( 'PL_HOOKS_PHP' ) ) {

				if( isset( $d['error'] ) && false != $d['error'] )
					echo "<p><span style='color:red' >This hook generated a PHP Fatal Error the last time it was executed.</span><br >The error reported was: <strong>{$d['error']}</strong></p>";

				if( isset( $d['php'] ) && true === $d['php'] )
					$check = 'checked';
				else
					$check = 'unchecked';
				echo "<br /><input type='checkbox' name='php' " . $check . " /> Enable PHP for this hook.";			
			}
			echo "<br /><input  name='priority' value='{$d['priority']}' /> Priority. (default is 10)";
			printf( '<br /><input name="page_id" value="%s" /> Only show on these pages/posts (coma seperated list of IDs).', implode( ',', $d['page_id'] ) );
			echo '<p><input type="submit" name="hooks-post" value="Save Hook" class="button-primary" />';
